...
public class CalendarQuickstart {
    ...
    public static void check(JPanel panelContainerSetTemperature, JLabel listSchedule, Values values, Rele rele, Sensor sensor, JLabel lblTemperature, JButton btnOnoff, JPanel panelContainerSchedule) throws IOException {
	...
	Events events = service.events().list(calendarId).setMaxResults(10).setTimeMin(now).setTimeMax(now1m).setOrderBy("startTime").setSingleEvents(true).execute();
	...
	if (items.size() == 0) {
	    ...
	    for (Event event : items) {
			...
			textEventi += event.getSummary() == null ? "" : event.getSummary() + " " + valnow + " " + valnowEnd + "<br/>";
	    }
	    listSchedule.setText(textEventi + "</html>");
    }
	
	Events eventsSingle = service.events().list(calendarId).setMaxResults(1).setTimeMin(now).setTimeMax(now1min).setOrderBy("startTime").setSingleEvents(true).execute();
	...
	if (itemsSingle.size() == 0) {
	    ...
	    {
	    	rele.set(Rele.CALDAIA, 0); //altrimenti spengo
		...
		System.out.printf("%s (%s)\n", itemsSingle.get(0).getSummary(), start.toString());
		
		Pattern pattern = Pattern.compile("[+-]?\\d*\\.?\\d+");
		Matcher matcher = pattern.matcher(eventsSingle.getSummary() == null ? "" : eventsSingle.getSummary());
        ...
        values.setToggleButton(false); //forzo il bottone a spento se sono in modalita calendario
        btnOnoff.setText("ON");
